...
void die(int error_code, const char *fmt, ...) {
  fflush(stdout);
  va_list ap;
  fprintf(stderr, "*** [maf] error code %i \n", error_code);
  fputs_unlocked("*** [maf] ", stderr);
  va_start(ap, fmt);
...
void print_statistics() {
  ...
  msg(1,"Statistics");
  msg(1,"==========================================================");
  msg(1,"  maximum resident set size:     %15.2f MB",
    maximum_resident_set_size() / static_cast<double>((1<<20)));
  msg(1,"  used time for initializing:    %15.2f seconds",
      init_time);
  msg(1,"  used time for reseting:        %15.2f seconds",
      reset_time - init_time);
  msg(1,"  total process time:            %15.2f seconds",  process_time());
